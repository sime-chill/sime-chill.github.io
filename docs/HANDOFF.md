...
- 当前开发目录：`E:\codex_work\personal-site-redesign`（WSL：`/mnt/e/codex_work/personal-site-redesign`）
- 当前开发分支：`redesign/acad-homepage`
- 盘点时 HEAD：`b2f09dd`
- Git 主工作区：`/home/wsl_hyh/sime-chill.github.io`，盘点时检出 `master`；该工作区本地提交/远端跟踪状态较旧，不是当前设计源。
- 最近一次 GitHub Pages workflow 在 `b2f09dd` 成功；这说明远端 `master` 已包含当前学术主页设计，即使本地 `origin/master` 尚未刷新。
...
1. 先在当前 worktree 运行 `git status --short`，不要覆盖尚未提交的 Logo 与文档改动。
